...
const Search = () => {
    ...
    const products: Product[] = 'hasValue' === ProductsLodable.state ? ProductsLodable.contents : []
    ...
    const goSearchList = (event: any) => {
        if (40 === event.keyCode) {
            ...
            let $next = event.target.nextElementSlibing
            ...
            event.preventDefault()
            let $next = event.target.parentElement.nextElementSlibing
            if (!$next) {
                return
            }
            $next.querySelector($searchedItem).focus()
    ...
    useEffect(() => {
        setFilterItems 
        products.filter((Selm) => {
            if (search === '') return 
            return Selm.title.toLowerCase().includes(search.toLowerCase())
        }),
    }, [search, products])
